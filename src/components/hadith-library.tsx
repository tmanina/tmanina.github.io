...
export function HadithLibrary({ onBack }: HadithLibraryProps) {
    ...
    // State for temporary "new" badge (visible for 2 days)
    const [showNewBadge, setShowNewBadge] = React.useState(false)
    React.useEffect(() => {
        const key = 'hadithLibraryNewBadgeTimestamp'
        const stored = localStorage.getItem(key)
        const now = Date.now()
        if (!stored) {
            // First time: store timestamp and show badge
        ...
        if (diffDays < 2) {
            ...
            // Optionally clean up old timestamp
    ...
    return (
        <div className="hadith-library animate__animated animate__fadeIn">
            ...
            <div className="library-header text-center">
                {showNewBadge && (
                    <div className="new-badge mb-2">جديد</div>
                )}
